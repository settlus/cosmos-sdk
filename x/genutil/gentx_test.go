...
func (suite *GenTxTestSuite) SetupTest() {
	...
	suite.msg1, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk1.Address()), pk1, amount, desc, comm, one, sdk.ZeroInt(), sdk.ZeroDec())
	suite.NoError(err)
	suite.msg2, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk2.Address()), pk1, amount, desc, comm, one, sdk.ZeroInt(), sdk.ZeroDec())
	suite.NoError(err)
}
