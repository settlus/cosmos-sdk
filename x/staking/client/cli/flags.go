...
func FlagSetMaxDelegation() *flag.FlagSet {
	fs := flag.NewFlagSet("", flag.ContinueOnError)
	fs.String(FlagMaxDelegation, "", "The maximum delegation on the validator")
	return fs
}

func FlagSetProbonoRate() *flag.FlagSet {
	fs := flag.NewFlagSet("", flag.ContinueOnError)
	fs.String(FlagProbonoRate, "", "Probono rate for the validator, between 0 and 1")
	return fs
}
